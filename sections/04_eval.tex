...
为评估重建语义建图的精度,我们在数据集点云中进行采样,使用采样点的坐标在完成建图的模型中进行推理得到采样点语义。随后我们将其与采样点数据集的label进行比较得到准确率等指标。
\subsection{实验准备}
\subsubsection{数据集}
...
\begin{table}[htbp]
    ...
    \begin{tabular}[htbp]{llccccc}
        \toprule
        \multicolumn{2}{l}{方法} & $\mathbf{Comp.}$ & $\mathbf{Acc.}$ & $\mathbf{C-l1}$ &  $\mathbf{Comp. Ratio}$ &$\mathbf{F1-score}$\\
        \midrule
        \multicolumn{2}{l}{Voxblox} & 7.1 & 1.8 & 4.8 &84.0&90.0\\
        \multicolumn{2}{l}{VDB Fusion} & 6.9&1.3&4.5&90.2&94.1 \\
        \multicolumn{2}{l}{SHINE-Mapping} & 3.2&1.1&2.9&95.2&95.9 \\
        \multicolumn{2}{l}{Vox Fusion} & 0.2 \\
        \midrule
        \multicolumn{2}{l}{Ours+bce} & 0.2 \\
        \bottomrule
    \end{tabular}
\end{table}
\subsubsection{真实场景}
\begin{table}[htbp]
    ...
    \begin{tabular}[htbp]{llccccc}
        ...
        \multicolumn{2}{l}{方法} & $\mathbf{Comp.}$ & $\mathbf{Acc.}$ & $\mathbf{C-l1}$ &  $\mathbf{Comp. Ratio}$ &$\mathbf{F1-score}$\\
        \midrule
        \multicolumn{2}{l}{Voxblox} & 14.9 & 9.3 &12.1 &87.8&87.9\\
        \multicolumn{2}{l}{VDB Fusion} & 12.0&6.9&9.4&91.3&92.6 \\
        \multicolumn{2}{l}{SHINE-Mapping} & 10.0&6.7&8.4&93.6&93.7 \\
        \multicolumn{2}{l}{Vox Fusion} & 30 & 123\\
        \midrule
        \multicolumn{2}{l}{Ours+bce} & 0.2 \\
        \bottomrule
    \end{tabular}
...
\begin{table}[htbp]
    \centering
    \caption{kitti语义建图指标}\label{kittimetrics}
    \begin{tabular}[htbp]{llcccc}
        \toprule
        \multicolumn{2}{l}{数据集} & 体素大小(m) & 采样间距(m) & 截断距离(m) &最大碰撞体素数 \\
        \midrule
...
\begin{table}[htbp]
    \centering
    \caption{不同方法在Mai City数据集01序列上建图的内存消耗。由于NICE-SLAM使用了4个特征网格,每一个特征网格对地图所有区域分配内存,因此内存消耗巨大。本文方法只使用一个特征网格,且只对观察到的地图区域分配体素与特征,使用较小的内存可以达到精细度更高的建图效果。}\label{memory}
    \begin{tabular}[htbp]{llcc}
        \toprule
        \multicolumn{2}{l}{方法} &解码器 & 地图特征\\
        \midrule
        \multicolumn{2}{l}{SHINE-Mapping} & 0.56MB & 7.09MB\\
        \multicolumn{2}{l}{NICE-SLAM} & 0.22MB & 8GB\\
        \midrule
