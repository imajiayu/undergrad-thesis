...
自动驾驶仿真系统需要高精度的地图,包括道路与车辆的几何信息等帮助其对环境进行分析与做出准确的决策。语义信息可以使系统更好的理解环境,当系统独立的进行语义分割与预测任务时,存储在地图中的语义信息可以作为监督信息对系统进行评估。在车辆行进过程中动态的对地图进行创建与更新也是一个自动驾驶系统所以需要的。针对以上问题,本文设置了以下定性与定量实验以验证\ref{algorithm}章中提出的优化算法的有效性。同时证明在建图精细度与时间空间代价上,相较于传统建图算法与其他神经隐式表达建图算法,本文在不同场景下具有优势:
\begin{itemize}
...
\end{table}
\subsubsection{真实场景}
本文使用5种方法在KITTI数据集Seq00序列的前200帧上进行建图。实验结果如图\ref{kittiresult}所示。可以看到本文方法在建图效果上媲美SHINE-Mapping与VDB Fusion,以相同的建图速度和更低的分辨率达到了相同的效果。本文使用的优化方法在地图细节处理上效果优于Vox-Fusion,如车辆,墙壁等细节。实验证明本文方法可以在真实自动驾驶数据集上对室外大规模地图进行实时建图,在保留细节的同时保持地图的连续性。
\begin{figure}[htbp]
	\centering
    (a)
	\begin{minipage}{0.322\linewidth}
...
\begin{table}[htbp]
    ...
    \end{tabular}
\end{table}
本文在KITTI数据集上进行了语义建图精度评估实验。如表\ref{kittimetrics}所示,在KITTI数据集的8个序列上,本文对每个关键帧采样1000个点云,使用训练完成的模型与解码器进行推理得到采样点的语义标签,并与SemanticKITTI数据集的语义标签进行比较以计算语义建图精度。结果显示在室外大规模场景下,本文语义建图方法可以达到约90\%的准确率。将预测出的语义标签映射为不同颜色后的效果如图\ref{semresult}所示。这些精确的语义信息可以为自动驾驶系统做出决策提供关键信息。
